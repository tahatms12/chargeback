...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  url.searchParams.set('redirectOrigin', url.origin);
...
export default function AuthLogin() {
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', maxWidth: 420, margin: '80px auto', padding: '0 24px' }}>
      <h1 style={{ fontSize: 24, marginBottom: 8 }}>Production Queue Communicator</h1>
      ...
      <Form method='post'>
        ...
        />
        {data?.error && (
          <p style={{ color: '#dc2626', fontSize: 14, marginBottom: 12 }}>{data.error}</p>
        )}
